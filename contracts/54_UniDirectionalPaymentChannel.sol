...
/// @title Payment channels allow participants to repeatedly transfer Ether off chain.
/// @author solidity by example
/// @notice - Alice deploys the contract, funding it with some Ether.
...
/// @notice This is called a uni-directional payment channel since the payment can go only in a single direction from Alice to Bob.      
...
contract UniDirectionalPaymentChannel is ReentrancyGuard {
    ...
    /// @notice for the receiver to claim
    ...
    /// @notice for the sender to close the contract
    function cancel() external {
        require(msg.sender == sender, "!sender");
        require(block.timestamp >= expiresAt, "!expired");
        // self destruct will send ether balance to the address in self destruct
        selfdestruct(sender);
    }
}
